/* --------------------------------------------------------------------------
  SCOPO
  Calcolo della somma di n numeri REALI utilizzando la STRATEGIA 3
...
int main (int argc, char **argv)
{
	...
	if (menum==0)
	{
        ...
        for(i=0; i<n; i++)
		{
			...
			*(vett+i)=(float)rand()/RAND_MAX*5.0-2.0;
		}
    ...
    else
    {
		...
		/*fase di ricezione*/
		MPI_Recv(vett_loc,nloc,MPI_DOUBLE,0,tag,MPI_COMM_WORLD,&info);
	}// end if
	...
	for(i=0;i<passi;i++)
	{
		...
		// Se il resto � MINORE a 2^i, il processore menum comunica con menum+DIST
		if(r<potenze[i])
		{
			// calcolo dell'id del processore a cui spedire la somma locale e da cui 				ricevere la sua somma parziale
			...
			recvBy=sendTo;
			MPI_Recv(&tmp,1,MPI_DOUBLE,recvBy,tag,MPI_COMM_WORLD,&info);
			
		}
		else // se il resto � MAGGIORE o UGUALE a 2^i, il processore menum 					comunica con menum-DIST
		{
